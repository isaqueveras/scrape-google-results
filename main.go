...
func GoogleScrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count int) (aa []SearchResult, err error) {
...
func main() {
	...
	fmt.Print("Informe a palavra da pesquisa: ")
	fmt.Scanln(&searchTerm)

	res, err := GoogleScrape(searchTerm, "com", "en", nil, 1, 20)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
